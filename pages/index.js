...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // 1. REKALKULASI STATUS BERDASARKAN RUMUS RSK (OCCUPANCY)
  ...
  const stats = useMemo(() => {
    ...
    data.forEach((item) => {
      ...
      // Grouping WOK -> STO untuk Tabel Occupancy
      ...
      wokMap[wok].is_total += item.is_total;
      wokMap[wok].used += item.used;
      wokMap[wok].avai += item.avai;

      wokMap[wok].stos[sto].odp_count += 1;
      wokMap[wok].stos[sto].is_total += item.is_total;
      wokMap[wok].stos[sto].used += item.used;
      wokMap[wok].stos[sto].avai += item.avai;
    });

    // Normalisasi Chart ke Persentase (0 - 100%)
          ...
          <div className="xl:col-span-6 space-y-3">
            ...
            <div className="bg-white rounded shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="grid grid-cols-4 border-t border-slate-200 bg-[#f8fafc] py-1.5 text-center text-[10px] font-bold text-slate-700">
                ...
                <div className="flex items-center justify-center space-x-1.5">
                  <span className="w-5 h-2.5 bg-[#15803d] rounded-sm inline-block"></span>
                  <span>&lt;60%</span>
                </div>
                <div className="flex items-center justify-center space-x-1.5">
                ...
                <div className="flex items-center justify-center space-x-1.5">
                  <span className="w-5 h-2.5 bg-[#ea580c] rounded-sm inline-block"></span>
                  <span>&lt;99%</span>
                </div>
              </div>
            </div>

            {/* STACKED BAR CHART ODP SHARE KABUPATEN */}
            <div className="bg-white rounded shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="p-3">
                ...
                <div className="flex justify-center items-center space-x-3 mt-1 text-[9px] font-bold text-slate-500">
                  <span className="flex items-center"><div className="w-2.5 h-2.5 bg-black mr-1 rounded-sm"></div>BLACK</span>
                  <span className="flex items-center"><div className="w-2.5 h-2.5 bg-[#15803d] mr-1 rounded-sm"></div>GREEN</span>
                  <span className="flex items-center"><div className="w-2.5 h-2.5 bg-[#facc15] mr-1 rounded-sm"></div>YELLOW</span>
                  <span className="flex items-center"><div className="w-2.5 h-2.5 bg-[#ea580c] mr-1 rounded-sm"></div>ORANGE</span>
                  <span className="flex items-center"><div className="w-2.5 h-2.5 bg-[#dc2626] mr-1 rounded-sm"></div>RED</span>
                </div>
              </div>
            </div>
          </div>
                    ...
                    {Object.entries(stats.wokMap).map(([wokName, wokData]) => (
                      <React.Fragment key={wokName}>
                        {Object.entries(wokData.stos).map(([stoName, stoData], j) => {
                          ...
                          // Penentuan warna background % OCC mengikuti rumus
                          let occBg = 'bg-[#86efac] text-emerald-950 font-bold'; // Green
                          if (occPercent === 0) occBg = 'bg-slate-200 text-slate-900 font-bold';
                          else if (occPercent >= 99) occBg = 'bg-[#f87171] text-red-950 font-bold'; // Red
                          else if (occPercent >= 85) occBg = 'bg-[#fed7aa] text-orange-950 font-bold'; // Orange
                          else if (occPercent >= 60) occBg = 'bg-[#fef08a] text-yellow-950 font-bold'; // Yellow
                          ...
                          return (
                            <tr key={stoName} className="hover:bg-slate-50 border-b border-slate-200">
                              ...
                              <td className="p-1 border border-slate-300 bg-slate-50 font-medium">
                                {avaiPercent.toFixed(1)}%
                              </td>
                            </tr>
                          );
                        })}

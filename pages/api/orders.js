...
// Daftar Kata Kunci Lookup Fallout Reason
...
// Helper Lookup Normalisasi Fallout Reason
function normalizeFalloutReason(rawVal) {
  ...
  for (const kw of FALLOUT_KEYWORDS) {
    ...
    // Word boundary regex agar tidak match substring kata lain
...
export default function OrdersPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const enriched = (data || []).map((row) => {
          ...
          // Lookup Fallout Reason Otomatis
  ...
  // ================= 1. PIVOT 1: WOK & STO vs ORDER DURATION =================
  ...
  // ================= 3. PIVOT 3: DURATION -> FALLOUT REASONS (HASIL LOOKUP BERSIH) =================
  const pivotFallout = useMemo(() => {
    const tree = {};
    let totalAll = 0;

    filteredOrders.forEach((o) => {
      if (!o.fallout_reason_clean) return; // Hanya order yang memiliki fallout
      const dur = o.order_duration_cat;
    ...
    <Sidebar>
      ...
      <div className="max-w-[1450px] mx-auto space-y-3">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-3 sm:gap-4">
          ...
          {/* PIVOT 2: WOK & STO vs ORDER STATUS */}
          <div className="bg-white border border-slate-300 shadow-xs rounded overflow-hidden">
            <div className="bg-[#0f172a] text-white p-2 flex justify-between items-center text-xs font-black uppercase">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-3 sm:gap-4">
          ...
          {/* PIVOT 3: DURATION -> FALLOUT REASON (HASIL LOOKUP STANDAR) */}

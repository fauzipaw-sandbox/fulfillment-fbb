...
// Load Komponen Peta Analisis Khusus secara Dinamis (No SSR)
...
export default function AnalysisPage() {
  ...
  const [radiusLimit, setRadiusLimit] = useState(250); // Default 250 Meter

  // Search & Target Terpilih (ODP atau Order)
  const [searchInput, setSearchInput] = useState('');
  const [suggestions, setSuggestions] = useState([]);
  const [selectedTarget, setSelectedTarget] = useState(null); // { type: 'ODP'|'ORDER', name, lat, lon, ... }
  ...
  const cleanOdpList = useMemo(() => {
    return (rawOdpData || []).filter((d) => d.latitude && d.longitude && d.odp_name);
  }, [rawOdpData]);

  // Bersihkan data Orders dengan tikor KP/latlon
  ...
  // Handle Input Pencarian ODP / Order
  ...
  // Hitung Data Kombinasi dalam Radius < 250m dari Target Terpilih
  const nearbyCombinedData = useMemo(() => {
    ...
    // 1. Cari ODP di sekitar target dalam radius < 250m
    ...
    // 2. Cari Order Pelanggan di sekitar target dalam radius < 250m
  ...
  // Sorting & Pencarian Tabel Kombinasi Bawah
  ...
  return (
    <Sidebar>
      ...
      <div className="max-w-[1450px] mx-auto space-y-3">
        ...
        {showUploader && (
          <div className="transition-all duration-300">
            <Uploader
              ...
              onUploadOrderSuccess={reloadAll}
            />
          </div>
        )}
        ...
        {/* Info Banner Target Radius Terpilih */}
        {selectedTarget ? (
          ...
          <div className="bg-slate-100 border border-slate-300 p-2 rounded text-[11px] text-slate-600 font-semibold flex items-center justify-between">
            <span>💡 <strong>Tips:</strong> Ketik nama ODP / Order ID di kotak pencarian atau klik titik marker pada peta untuk membuat <strong>lingkaran radius &lt; {radiusLimit}m</strong> dan melihat ODP terdekat serta jaraknya ke pelanggan.</span>
          </div>
        )}

        {/* ================= 1. MAPS LUAS ODP & ORDER ANALYSIS ================= */}
        <div className="bg-white border border-slate-300 shadow-sm rounded-lg overflow-hidden h-[420px] sm:h-[500px]">
        ...
        {/* ================= 2. TABEL KOMBINASI RADIUS ODP & ORDER TERDEKAT ================= */}
                    ...
                    return (
                      ...
                      >
                        ...
                        <td
                          ...
                          onClick={() => handleSelectTarget({ ...row, targetType: row.rowType, odp_name: row.name, order_id: row.name, lat: row.latitude, lon: row.longitude, displayName: row.name })}
